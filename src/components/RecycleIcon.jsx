/* RecycleIcon — the classic Möbius recycling symbol, as a themeable inline SVG.
   The path is lifted verbatim from recycling_symbol.svg (in this folder); the only
   change is fill="currentColor" so it inherits the surrounding text colour (e.g. the
   Build Queue's recycle button, which greys by default and brightens on hover).
   viewBox is the source art's; the default 13px size fits the round .qrecycle button. */

export function RecycleIcon({ size = 13, className, title }) {
  return (
    <svg
      className={className}
